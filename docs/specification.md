...
エンチャント効果の公式アイテムページでは「エンチャント可能な装備」を件数付きで一覧表示します。Web版の `item-enchantment-targets.json` をアイテムIDと完全一致名で逆引きし、装備IDで重複を除きます。装備名は公開name-mapで解決し、JRO Searchの詳細へリンクします。逆引きデータは成功時のみパネル内で5分間キャッシュし、ページ切り替えごとの再取得を避けます。

公式のアイテム詳細ページを開いて「検索補助」を選択すると、JRO Searchのエンチャント情報を表示します。エンチャント種別ごとの折り畳み内にNPC、素材、成功率・失敗時の効果、精錬などの条件、対象スロットと候補を表示します。アイテム名とID解決済みの候補はJRO Searchの詳細へのリンクです。

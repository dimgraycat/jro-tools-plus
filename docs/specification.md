...
Side Panel のヘッダー上段は「検索・所持Zeny・更新履歴」の3タブです。初期表示は検索補助です。「検索」の選択中のみ2段目に「検索補助・お気に入り・検索履歴」を表示します。他の上段タブから戻ると、直前に選択した検索内のページへ戻ります。各段は320px幅でも横一列に収めます。検索履歴は既存の閲覧履歴の表示名であり、保存内容・同期方式は変更しません。

### 検索補助

コスたまの `revivals` がある場合は、収録名の下に「復刻・再販履歴」を新しい順で折り畳み表示し、期間と公式リンクを示します。個別詳細の `costama_point_exchanges` は「復刻コスたま ポイント交換」として必要ポイント、交換期間、公式リンクを表示します。抽選対象の収録件数には混ぜません。どちらも公開Webデータの反映後に取得できます。

現在のアイテムの「衣装の収録情報（コスたま）」と「ラグ缶の収録情報」を件数付きで表示します。公開 `package-index.json` の `costama` / `ragcan` をアイテムIDで照合し、再収録を含め公開データの順序で表示します。収録名から公式収録ページへ新しいタブで移動できます。公式ホスト以外のURLはリンクにしません。収録データは成功時のみパネル内で5分間キャッシュします。

エンチャント効果の公式アイテムページでは「エンチャント可能な装備」を件数付きで一覧表示します。Web版の `item-enchantment-targets.json` をアイテムIDと完全一致名で逆引きし、装備IDで重複を除きます。装備名は公開name-mapで解決し、RO公式の詳細へリンクします。横の虫眼鏡アイコンはJRO Searchの詳細を開きます。逆引きデータは成功時のみパネル内で5分間キャッシュし、ページ切り替えごとの再取得を避けます。

公式のアイテム詳細ページを開いて「検索補助」を選択すると、JRO Searchのエンチャント情報を表示します。エンチャント種別ごとの折り畳み内にNPC、素材、成功率・失敗時の効果、精錬などの条件、対象スロットと候補を表示します。アイテム名とID解決済みの候補はRO公式の詳細へのリンクです。横の虫眼鏡アイコンはJRO Searchの詳細を開き、ホバー・キーボードフォーカス時に「JRO Searchで開く」と表示します。どちらも新しいタブで開きます。

- Webページを同時に開く必要はありません。公開済みの詳細JSONをIDのmodulo64で選択して読み込み、Web側のページや公式本文を埋め込みません。
- 検索補助タブを表示中のみ取得し、公式アイテムの切り替えに追従します。モンスターや対象外のページでは案内だけを表示します。
- 取得のタイムアウトは10秒です。通信失敗は再試行でき、未登録アイテム・エンチャント情報なしとは区別します。古い取得結果は新しいページの表示を上書きしません。
- 取得・データ整形は `tools/lib/search-assist.ts`、Chromeタブ追従・DOM表示は `tools/ts/search-assist.ts` に分離します。共有お気に入りや閲覧履歴の保存処理は変更しません。

- `所持Zeny`: 所持 Zeny 収集画面
- `お気に入り`: 保存済みの一覧・名前検索・セット絞り込み・現在の公式ページの登録／解除
- `検索履歴`: 最近開いた公式詳細・Webプレビューの一覧とお気に入り操作（保存するデータは閲覧履歴）
